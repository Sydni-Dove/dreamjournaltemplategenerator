...
- `detectRepeatedSimpleSectionBlocks` — detects repeated heading pattern. It should activate only when actual normalized heading labels repeat (for example `Dream` twice or `Brainstorm` twice). Distinct one-time headings like `Dream`, `Brainstorm`, `Interpretation`, `Application` should remain `labeled_sections`. This description is architectural intent only; it is not evidence that any specific regression is already fixed.
